...
#else
	mpchunk = MEM_malloc(sizeof(BLI_mempool_chunk) + (size_t)pool->csize);
#endif
                                ...
                                unsigned int pchunk, unsigned int flag)
{
	...
	pool = MEM_malloc(sizeof(BLI_mempool));
...
{
	void **data = MEM_malloc((size_t)pool->totused * sizeof(void *));
...
{
	char *data = MEM_malloc((size_t)(pool->totused * pool->esize));
