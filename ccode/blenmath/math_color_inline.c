...
static void F3TOCHAR3(float *f, char *c) {
	unsigned char *c2 = (unsigned char*) c;

	c2[0] = (unsigned char) (f[0]*255.0f);
	c2[1] = (unsigned char) (f[1]*255.0f);
	c2[2] = (unsigned char) (f[2]*255.0f);
}

static void F4TOCHAR4(float *f, char *c) {
	unsigned char *c2 = (unsigned char*) c;

	c2[0] = (unsigned char) (f[0]*255.0f);
	c2[1] = (unsigned char) (f[1]*255.0f);
	c2[2] = (unsigned char) (f[2]*255.0f);
	c2[3] = (unsigned char) (f[2]*255.0f);
}

#define UNUSED(a) a
#define LIKELY(b) b

#define FTOCHAR(fl) ((unsigned char)(fl*255.0f))
#define FTOUSHORT(fl) ((unsigned char)(fl*65535.0f))
