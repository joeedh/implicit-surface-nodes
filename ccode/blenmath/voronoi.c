...
{
	VoronoiEdge *edge = MEM_calloc(sizeof(VoronoiEdge));
...
{
	VoronoiParabola *parabola = MEM_calloc(sizeof(VoronoiParabola));//, "voronoi parabola");
...
{
	VoronoiParabola *parabola = MEM_calloc(sizeof(VoronoiParabola));//, "voronoi parabola site");
...
static void voronoi_checkCircle(VoronoiProcess *process, VoronoiParabola *b)
{
	...
	event = MEM_calloc(sizeof(VoronoiEvent));//, "voronoi circle event");
...
static void voronoi_clampEdges(ListBase *edges, int width, int height, ListBase *clamped_edges)
{
	...
	edge = edges->first;
	while (edge) {
		VoronoiEdge *new_edge = MEM_calloc(sizeof(VoronoiEdge));//, "clamped edge");
...
static void voronoi_createBoundaryEdges(ListBase *edges, int width, int height)
{
	...
	for (i = 0; i < 4; i++) {
		while (voronoi_getNextSideCoord(edges, coord, dim, dir, next_coord)) {
			VoronoiEdge *edge = MEM_calloc(sizeof(VoronoiEdge));//, "boundary edge");
...
void BLI_voronoi_compute(const VoronoiSite *sites, int sites_total, int width, int height, ListBase *edges)
{
	...
	for (i = 0; i < sites_total; i++) {
		VoronoiEvent *event = MEM_calloc(sizeof(VoronoiEvent));//, "voronoi site event");
                                         ...
                                         int *triangulated_points_total)
{
	...
	if (*triangulated_points) {
		*triangulated_points = MEM_realloc(*triangulated_points,
		                                    sizeof(VoronoiTriangulationPoint) * (*triangulated_points_total + 1));
	}
	else {
		*triangulated_points = MEM_calloc(sizeof(VoronoiTriangulationPoint));//, "triangulation points");
	}
...
static void voronoi_addTriangle(int v1, int v2, int v3, int (**triangles)[3], int *triangles_total)
{
	...
	if (*triangles) {
		*triangles = MEM_realloc(*triangles, sizeof(int[3]) * (*triangles_total + 1));
	}
	else {
		*triangles = MEM_calloc(sizeof(int[3]));//, "trianglulation triangles");
	}
